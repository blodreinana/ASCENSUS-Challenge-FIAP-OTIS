...
export default function RegisterPage() {
  ...
  const { registrarTecnico } = useStore();
  ...
  const handleRegister = (e) => {
    ...
    const sucesso = registrarTecnico(username, password);
    if (sucesso) {
      navigate("/login"); 
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-50 via-sky-50 to-indigo-100">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="text-center mt-6">
            <span className="text-zinc-600 text-sm">Já tem uma conta? </span>
            <Link to="/login" className="text-sky-500 hover:text-sky-600 text-sm font-medium transition">
              Faça login
            </Link>
        </div>
      </div>
